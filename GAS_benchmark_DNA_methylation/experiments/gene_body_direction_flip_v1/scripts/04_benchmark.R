...
# ============================================================
# 04b_benchmark_with_distributions_gene_body_direction_flip_v1.R
# Purpose:
...
results_dir <- file.path(EXP_ROOT, "results", "ArchR_paper_benchmark_gene_body_direction_flip_v1_strict")
...
heatmap_prefix <- file.path(figure_dir, "Methylation_rank_heatmap_gene_body_direction_flip_v1_strict")
violin_prefix <- file.path(figure_dir, "Methylation_violin_gene_body_direction_flip_v1_strict")
...
plot_df_hm <- rank_df %>%
  dplyr::mutate(family_label = dplyr::recode(family, !!!family_label_map, .default = family))
mat_raw <- as.matrix(plot_df_hm[, c(
...
ann_df <- data.frame(family = plot_df_hm$family_label[ord], row.names = rownames(mat), stringsAsFactors = FALSE)
...
readr::write_csv(
  tibble::tibble(model_id = plot_df_hm$model_id[ord], model = plot_df_hm$model[ord], family = plot_df_hm$family_label[ord], mean_rank = mean_rank[ord]),
  paste0(heatmap_prefix, "_model_id_map.csv")
)
readr::write_csv(
  tibble::tibble(test_id = 1:4, test_name = tests_to_rank),
