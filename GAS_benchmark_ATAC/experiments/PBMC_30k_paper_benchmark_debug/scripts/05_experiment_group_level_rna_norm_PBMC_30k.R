...
rna_group_raw_rds <- file.path(official_results_dir, "Save-KNN-Groups-scRNA-Matrix.raw_counts.rds")
...
results_dir <- file.path(exp_root, "results", "celltype_diffgenes_groupnorm_v1")
dir.create(results_dir, recursive = TRUE, showWarnings = FALSE)

summary_csv <- file.path(results_dir, "model_corr_summary_celltype_diffgenes_groupnorm.csv")
rank_csv <- file.path(results_dir, "model_rank_summary_celltype_diffgenes_groupnorm.csv")
test_map_csv <- file.path(results_dir, "test_id_map_celltype_diffgenes_groupnorm.csv")
...
cat("=== PBMC Paper-Style Benchmark Debug: group-level RNA normalization ===\n")
cat("Official input dir: ", official_results_dir, "\n", sep = "")
...
matRNA_raw <- readRDS(rna_group_raw_rds)
rna_feature_names <- get_feature_order_from_arrows(proj, "GeneIntegrationMatrix")
matRNA_raw <- assign_dimnames_or_stop(
  mat = matRNA_raw,
  feature_names = rna_feature_names,
  group_names = colnames(matRNA_raw),
  matrix_name = "GeneIntegrationMatrix.raw_group_counts"
)
matRNA <- normalize_log2(matRNA_raw)
...
run_notes <- tibble::tibble(
  dataset_tag = dataset_tag,
  input_group_matrix = rna_group_raw_rds,
  diff_gene_source = rna_markers_rds,
  var_gene_source = rna_var_tsv,
  grouping_used = "stored RNA$Group from processed Seurat object",
  rna_matrix_construction = "aggregate raw counts by group, then normalize_log2 at group level",
  diff_gene_params = "reuse saved markers from 03_prepare; top-n by group with min.pct=0.25 logfc.threshold=0.25",
  var_gene_params = "reuse saved variable genes from 03_prepare",
  n_var_genes = length(varGenes),
  n_diff_genes = length(diffGenes)
)
...
write_csv(run_notes, run_note_csv)
