...
input_dir <- file.path(exp_root, "results", "rna_pca_groups_v1")
...
rna_group_rds <- file.path(input_dir, "Save-KNN-Groups-scRNA-Matrix.rna_pca.rds")

summary_csv <- file.path(results_dir, "model_corr_summary_rna_pca_groups.csv")
rank_csv <- file.path(results_dir, "model_rank_summary_rna_pca_groups.csv")
test_map_csv <- file.path(results_dir, "test_id_map_rna_pca_groups.csv")
run_note_csv <- file.path(results_dir, "run_notes.csv")
...
cat("=== PBMC Paper-Style Benchmark Debug: RNA-PCA groups ===\n")
cat("Input aggregate dir: ", input_dir, "\n", sep = "")
...
model_paths <- tibble::tibble(
  ...
  file = file.path(input_dir, paste0(model_manifest$name, "_gene_by_group.rds"))
)
...
if (!("Group" %in% colnames(RNA[[]]))) {
  RNA$Group <- RNA$seurat_clusters
}
Idents(RNA) <- RNA$Group

varGenes <- read_tsv(rna_var_tsv, show_col_types = FALSE)$gene
varGenes <- head(varGenes[varGenes %in% rownames(matRNA)], 2000)
...
rank_df <- summary_rows %>%
  dplyr::mutate(
    across(all_of(test_cols), ~ dplyr::dense_rank(dplyr::desc(.x)), .names = "{.col}_rank"),
    mean_rank = rowMeans(dplyr::across(dplyr::ends_with("_rank")))
  ) %>%
  dplyr::mutate(
    Rank_Pearson_DiffGenes_GeneLvl_median = Pearson_DiffGenes_GeneLvl_median_rank,
    Rank_Pearson_DiffGenes_GroupLvl_median = Pearson_DiffGenes_GroupLvl_median_rank,
    Rank_Pearson_VarGenes_GeneLvl_median = Pearson_VarGenes_GeneLvl_median_rank,
    Rank_Pearson_VarGenes_GroupLvl_median = Pearson_VarGenes_GroupLvl_median_rank
  ) %>%
  dplyr::arrange(mean_rank, model_id)

test_map <- tibble::tibble(
  test_id = c("Test1", "Test2", "Test3", "Test4"),
  column_name = test_cols,
  label = c(
    "DiffGenes across genes",
    "DiffGenes across groups",
    "VarGenes across genes",
    "VarGenes across groups"
  )
)

write_csv(summary_rows, summary_csv)
write_csv(rank_df, rank_csv)
write_csv(test_map, test_map_csv)

write_csv(
  tibble::tibble(
    experiment = "rna_pca_groups_v1",
    description = "Use RNA-PCA low-overlap groups built from per-cell GeneIntegrationMatrix, then rerun paper-style benchmark with celltype-based diff genes.",
    n_groups = unique(summary_rows$n_groups),
    n_var_genes = unique(summary_rows$n_var_genes),
    n_diff_genes = unique(summary_rows$n_diff_genes)
  ),
  run_note_csv
)
